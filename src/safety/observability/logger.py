...
Monitor in a separate terminal with:
    ...
    tail -f data/agent.log | python -m json.tool    # pretty-printed
    tail -f data/agent.log | grep '"level": "ERROR"' # errors only
"""
...
class _SafeEncoder(json.JSONEncoder):
    ...
    Prevents crashes from third-party loggers (e.g. httpx, google-genai)
    that pass URL objects, Pydantic models, or other custom types as log args.
    """
...
class JsonFormatter(logging.Formatter):
    ...
    def format(self, record: logging.LogRecord) -> str:
        ...
        Returns:
            JSON string with timestamp, level, logger, message, and safe extras.
        """
        log_obj = {
...
def setup_logging(level: int = logging.INFO) -> None:
    ...
    Stdout output:
      - WARNING and above only, plain text with emoji prefix
      - Keeps the CLI interface clean for executives
    ...
    # Rotating file handler — full JSON structured logs
    ...
    for noisy in ("httpx", "httpcore", "google.auth", "urllib3", "google.cloud.bigquery"):
